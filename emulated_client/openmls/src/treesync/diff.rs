...
impl TreeSyncDiff<'_> {
    ...
    pub(crate) fn apply_own_update_path(    
        ...
        path: Vec<(ParentNodeIndex, ParentNode)>,
    ) -> Result<Vec<EncryptionKeyPair>, TreeSyncAddLeaf> {
    ...
    fn set_parent_hashes(
        ...
        let modified_leaves_owned: Vec<(LeafNodeIndex, TreeSyncLeafNode)> =
            self.diff
                .diff_leaves()
                .map(|(index, leaf)| (index, leaf.clone()))
                .collect();

        let modified_leaves: Vec<(LeafNodeIndex, &TreeSyncLeafNode)> =
            modified_leaves_owned
                .iter()
                .map(|(index, leaf)| (*index, leaf))
                .collect();
        self.update_tree_hashes(crypto, ciphersuite, modified_leaves)?;

        // For every copath node, compute the tree hash.
        let copath_tree_hashes = filtered_copath.into_iter().map(|node_index| {
            self.tree_hash(node_index)
        });
    ...
    pub(super) fn resolution(
        ...
        rightmost_full_leaf: &LeafNodeIndex,
        ...
        match node_index {
            ...
            TreeNodeIndex::Parent(parent_index) => {
                match self.diff.parent(parent_index).node() {
                    ...
                    None => {
                        let leftmost = self.diff.leftmost_child(parent_index);

                        if let TreeNodeIndex::Leaf(leaf_index) = leftmost {
                            if leaf_index.u32() > rightmost_full_leaf.u32() {
                                return vec![];
                            }
                        }
                        ...
                        resolution.append(&mut self.resolution(left_child, excluded_indices, rightmost_full_leaf));
                        resolution.append(&mut self.resolution(right_child, excluded_indices, rightmost_full_leaf));
                        resolution
                    }
                }
            }
    ...
    pub(crate) fn copath_resolutions(
        ...
        self.diff
            .copath(leaf_index)
            .into_iter()
            .map(|node_index| self.resolution(node_index, &HashSet::new(), &rightmost_full_leaf))
            .collect()
    }
    ...
    pub(super) fn verify_parent_hashes(
        ...
        for (parent_index, tree_sync_parent_node) in self.diff.parents() {
            if let Some(parent_node) = tree_sync_parent_node.node() {
                ...
                let (oth_left, oth_right) = if exclusion_list.is_empty() {
                    let oth_left = self.tree_hash(left_child)?;
                    let oth_right = self.tree_hash(right_child)?;
                ...
                let left_resolution = self.resolution(left_child, &exclusion_list, &rightmost_full_leaf);

                let right_resolution = self.resolution(right_child, &exclusion_list, &rightmost_full_leaf);
    ...
    pub(crate) fn into_staged_diff(
        ...
        let new_tree_hash = self.tree_hashes[root(self.diff.size()).usize()].clone();
    ...
    pub(crate) fn tree_hash(
        ...
        Ok(self.tree_hashes[node_index.usize()].clone())
    }

    pub(crate) fn populate_tree_hashes(
        &mut self,
        crypto: &impl OpenMlsCrypto,
        ciphersuite: Ciphersuite,
        node_index: TreeNodeIndex,
        ...
        match node_index {
            TreeNodeIndex::Leaf(leaf_index) => {
                let leaf: &TreeSyncLeafNode = self.diff.leaf(leaf_index);

                let result = if exclusion_list.contains(&leaf_index) {
                    TreeSyncLeafNode::blank().compute_tree_hash(crypto, ciphersuite, leaf_index)
                } else {
                    leaf.compute_tree_hash(crypto, ciphersuite, leaf_index)
                }?;

                let tree_index = TreeNodeIndex::Leaf(leaf_index);
                self.tree_hashes[tree_index.usize()] = result.clone();
            ...
            TreeNodeIndex::Parent(parent_index) => {
                ...
                let left_child = self.diff.left_child(parent_index);
                let left_hash =
                    self.populate_tree_hashes(crypto, ciphersuite, left_child, exclusion_list)?;
                // Compute right hash.
                let right_child = self.diff.right_child(parent_index);
                let right_hash =
                    self.populate_tree_hashes(crypto, ciphersuite, right_child, exclusion_list)?;

                let node = self.diff.parent(parent_index);

                let result =node.compute_tree_hash(crypto, ciphersuite, left_hash, right_hash, exclusion_list)?;

                let tree_index = TreeNodeIndex::Parent(parent_index);
                self.tree_hashes[tree_index.usize()] = result.clone();
                Ok(result)
            }
        }
    ...
    pub(crate) fn update_tree_hashes(
        &mut self,
        crypto: &impl OpenMlsCrypto,
        ciphersuite: Ciphersuite,
        modified_leaves: Vec<(LeafNodeIndex, &TreeSyncLeafNode)>
    ) -> Result<Vec<u8>, LibraryError> {

        let exclusion_list = HashSet::new();

        let mut node_queue: VecDeque<ParentNodeIndex> = VecDeque::with_capacity(modified_leaves.len());

        let num_leaves = self.leaf_count().max(MIN_TREE_SIZE);
        self.tree_hashes.resize(num_leaves as usize * 2 - 1, Vec::default());

        let mut modified_leaves = modified_leaves.clone();
        let blank_leaf = TreeSyncLeafNode::blank();
        
        // Add to "modified_leaves" all even positions added during the resize
        for i in (0..self.tree_hashes.len()).step_by(2) {
            if self.tree_hashes[i as usize].is_empty() {
                let leaf_index = LeafNodeIndex::new(i as u32 / 2);
                if !modified_leaves.iter().any(|(idx, _)| *idx == leaf_index) {
                modified_leaves.push((leaf_index, &blank_leaf));
                }
            }
        }

        for (index, leaf) in modified_leaves {

            let tree_index = TreeNodeIndex::Leaf(index);
            self.tree_hashes[tree_index.usize()] = leaf.compute_tree_hash(crypto, ciphersuite, index)?;

            if tree_index != root(self.diff.size()) {
                let parent = parent(tree_index);
                node_queue.push_back(parent);
            }
        }

        while let Some(parent_index) = node_queue.pop_front() {
            let node = self.diff.parent(parent_index);

            let hash = node.compute_tree_hash(
                crypto, 
                ciphersuite,
                self.tree_hashes[self.diff.left_child(parent_index).usize()].clone(),
                self.tree_hashes[self.diff.right_child(parent_index).usize()].clone(),
                &exclusion_list,
            )?; 

            let tree_index = TreeNodeIndex::Parent(parent_index);
            self.tree_hashes[tree_index.usize()] = hash;
        ...
        let root_hash = self.tree_hashes[root(self.diff.size()).usize()].clone();
    ...
    pub(crate) fn filtered_common_direct_path(
        &self,
        leaf_index_1: LeafNodeIndex,
        leaf_index_2: LeafNodeIndex,
    ) -> Vec<ParentNodeIndex> {
    ...
    pub(crate) fn root_hash(&self) -> &[u8] {
        &self.tree_hashes[root(self.diff.size()).usize()]
    }
}
