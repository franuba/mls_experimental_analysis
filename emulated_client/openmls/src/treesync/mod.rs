...
use crate::binary_tree::array_representation::{ParentNodeIndex, root as tree_root};
#[cfg(any(feature = "test-utils", test))]
...
impl TreeSync {
    ...
    fn populate_tree_hashes(
        ...
        self.tree_hashes.resize(self.tree_size().u32() as usize, Vec::new());
        ...
        diff.populate_tree_hashes(crypto, ciphersuite, tree_root(self.tree_size()), &HashSet::new())?;
    ...
    pub(crate) fn full_leave_members_opt(&self) -> impl Iterator<Item = (LeafNodeIndex, &[u8], &[u8])> + '_ {
        self.tree
            .leaves()
            // Filter out blank nodes
            .filter_map(|(index, tsn)| tsn.node().as_ref().map(|node| (index, node)))
            ...
            .map(|(index, leaf_node)| {
                    (
                    index,
                    leaf_node.encryption_key().as_slice(),
                    leaf_node.signature_key().as_slice(),
                    )
            })
    }
